...
def extract_insert_apch(file_name, rwy_dir, tables):
    ...
    for _, row in coding_df.iterrows():
        ...
        if bool(row[-1].strip()):
            ...
            vpa_nav_data = row[10].strip()
            if vpa_nav_data:
                vpa_nav_values = vpa_nav_data.split()
                if len(vpa_nav_values) == 3:
                    proc_des_obj.vpa_tch = vpa_nav_values[0]
                    proc_des_obj.nav_spec = " ".join(vpa_nav_values[1:])
            ...
            if len(data_parts) > 2:
                if data_parts[-1] != 'RNP APCH':
                    ...
                    data_parts.insert(6, data_to_insert)
                    data_parts.pop(0)
                    data_parts.pop(0)
                    data_parts.pop(-1)
                # print(data_parts)    
