...
def extract_insert_apch_08(file_name, rwy_dir, tables):
    ...
    for _, row in apch_data_df.iloc[1:].iterrows():
        ...
        if bool(row[-1].strip()):
            ...
            if  data_parts[0] == "-" or data_parts[0].replace('.', '', 1).isdigit() or data_parts[0] == "HM":
                if data_parts[0] == "-":
                ...
                #  print(proc_des_obj)
                 ...
                 if is_valid_data(data := row[3]):
                    if data == "Y":
                        ...
                        proc_des_obj.fly_over = False
                 sequence_number += 1
                elif len(data_parts) == 13:
                #  print(data_parts)
                 data_parts.insert(0, data_parts[5])
                 data_parts.pop(6)
                 data_parts.pop(-4)
                 waypoint_name = data_parts[2]
                 waypoint_obj = (
...
def extract_insert_apch_26(file_name, rwy_dir, tables):
    process_id = get_active_process_id()
    waypoint_tables = tables[1:]
    for waypoint_table in waypoint_tables:
        waypoint_df = waypoint_table.df
...
def main():
    ...
    for file_name in apch_coding_file_names:
        ...
        if rwy_dir == "08":
         extract_insert_apch_08(file_name, rwy_dir, tables)
        elif rwy_dir == "26":
          extract_insert_apch_26(file_name, rwy_dir, tables)
