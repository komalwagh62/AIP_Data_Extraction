...
def extract_insert_apch(file_name, rwy_dir, tables):
    ...
    sequence_number = 1
    for _, row in coding_df[1:].iterrows():
        row = list(row)
        if  row[-1] == 'RNP APCH':
            ...
            course_angle = row[4].replace("\n", "").replace("  ", "").replace(" Mag", "").replace(" True", "").replace(" / ", " ")
          ...
          if row[0].isdigit():
            ...
            # Flatten row by replacing -2 with unpacked split data and removing the last item (-1)
            row = row[:-2] + split_data + row[-1:]
            row.pop(-1)  
            
            if len(row) < 10:  # Skip rows with insufficient columns
             continue
            # print(row)
            waypoint_obj = None
            if is_valid_data(row[2]):
            ...
            proc_des_obj = ProcedureDescription(
                ...
                dst_time=row[5].strip() if is_valid_data(row[5]) else None,
                vpa_tch=row[9].strip() if is_valid_data(row[9]) else None,
                # Ensure row length is valid before accessing index 10
