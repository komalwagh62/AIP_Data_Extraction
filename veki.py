...
from model import session, Waypoint, Procedure, ProcedureDescription,TerminalHolding,AiracData
        ...
        if bool(row[-1].strip()):
            ...
            if len(data_parts) > 1:
                if data_parts[0].isdigit():
                    ...
                    data_parts.pop(-1)
                    data_parts.pop(-3)
                    data_parts.insert(10, data_parts[-3])
                    data_parts.pop(-4)
                    data_parts.insert(6, "-")
                else:
            ...
            # Check if we have exactly two angle values
                ...
                proc_des_obj = ProcedureDescription(
                    ...
                    role_of_the_fix=data_parts[10].strip()
