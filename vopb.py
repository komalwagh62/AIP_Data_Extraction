...
def extract_insert_apch(file_name, rwy_dir, tables):
    ...
    for _, row in coding_df.iterrows():
        ...
        if bool(row[-1].strip()):
            ...
            course_distance_data = row[4].strip()
            if course_distance_data:
                course_distance_values = course_distance_data.split()
                # print(len(course_distance_values))
                if len( course_distance_values) == 3:
                    proc_des_obj.course_angle = course_distance_values[0]
                    proc_des_obj.dst_time = " ".join(course_distance_values[1:])
                    proc_des_obj.vpa_tch = None  
            
            if is_valid_data(row[9]):
                    VPA = row[9].strip().split()  # Split on spaces or other delimiters
                    proc_des_obj.vpa_tch = "".join(VPA)  
            ...
            if len(data_parts) > 2:
                if data_parts[0] == 'RNP APCH':
                    data_parts.insert(-1, 'RNP APCH')
                    data_parts.pop(0)
                data_parts[-1],data_parts[-2] = data_parts[-2],data_parts[-1]
                data_parts.insert(-1, '')
                if (data_parts[0] == '20' or data_parts[0] == '30')  : 
                     data_parts.insert(-2, '')
                if len(data_parts) == 10:
                    data_parts.insert(6,'R')    
                course_distance_data = data_parts[4].strip()
                if course_distance_data:
                    course_distance_values = course_distance_data.split()
                    # print(len(course_distance_values))
                    if len( course_distance_values) == 3:
                        data_parts[4] = course_distance_values[0]
                        data_parts[5] = " ".join(course_distance_values[1:])
                        data_parts[-2]= ''
