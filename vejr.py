...
def extract_insert_apch(file_name, rwy_dir, tables):
    ...
    for waypoint_table in waypoint_tables:
        ...
        for _, row in waypoint_df.iterrows():
            ...
            # print(f"Extracted data: {extracted_data1} from row: {row[1]}")  # Debugging line
    ...
    for _, row in coding_df.iterrows():
        ...
        if bool(row[-1].strip()):
            ...
            course_distance_data = row[5].strip()
            ...
            if len(data_parts) > 2:
                if data_parts[0] == 'RNP APCH':
                    ...
                    data_parts.pop(0)
                data_parts[8],data_parts[7] = data_parts[7], data_parts[8]
                data_parts.insert(-1, '')
                data_parts.insert(-1, '')
                # print(data_parts)
